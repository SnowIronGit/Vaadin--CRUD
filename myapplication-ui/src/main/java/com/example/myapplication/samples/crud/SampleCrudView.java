...
public class SampleCrudView extends CssLayout implements View {
	...
	public SampleCrudView() {
		...
		//Shows a symbol if a book is already approved or not
		grid.getColumn("approvedStatus").setRenderer(new HtmlRenderer(),
				new StringToBooleanConverter(FontAwesome.CHECK_CIRCLE_O.getHtml(), FontAwesome.CIRCLE_O.getHtml()));

		//Creates a button for approving a book. Opens a new windows to approve a book
		grid.getColumn("approve").setRenderer((new ButtonRenderer(e -> 
		{
			ReleaseWindows sub = new ReleaseWindows(grid, e.getItemId());
