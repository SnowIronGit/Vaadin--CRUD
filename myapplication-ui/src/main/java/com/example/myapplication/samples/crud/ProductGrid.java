...
public class ProductGrid extends Grid {
	...
	private StringToEnumConverter availabilityConverter = new StringToEnumConverter() {
		@Override
		public String convertToPresentation(Enum availability, java.lang.Class<? extends String> targetType,
				Locale locale) throws Converter.ConversionException {
			String text = super.convertToPresentation(availability, targetType, locale);
	...
	public ProductGrid() {
		...
		setColumnOrder("id", "productName", "price", "currency", "approve", "approvedStatus", "availability",
				"stockCount", "category");

		// Show empty stock as "-"
		getColumn("stockCount").setConverter(new StringToIntegerConverter() {
			@Override
			public String convertToPresentation(Integer value, java.lang.Class<? extends String> targetType,
					Locale locale) throws Converter.ConversionException {
				if (value == 0) {
					return "-";
				}
		...
		getColumn("availability").setConverter(availabilityConverter).setRenderer(new HtmlRenderer());
	...
	public void setFilter(String filterString) {
		...
		if (filterString.length() > 0) {
			SimpleStringFilter nameFilter = new SimpleStringFilter("productName", filterString, true, false);
			SimpleStringFilter availabilityFilter = new SimpleStringFilter("availability", filterString, true, false);
			SimpleStringFilter categoryFilter = new SimpleStringFilter("category", filterString, true, false);
	...
	//get-method to get a specific Bean from a BeanItemContainer. Used for the ReleaseWindows mainly
	public Object getValue(Object object) {
		return getContainer().getItem(object).getBean();
	}
